    ...
    >
      <div examples class="space-y-6">
        @if (activeTab() === 'linear') {
          <app-doc-section
            ...
            description="Step-by-step registration with validation"
            [codeExample]="linearCode"
          >
        ...
        @if (activeTab() === 'nonlinear') {
          <app-doc-section
            title="Non-linear Wizard"
            description="Jump to any step without completing previous steps"
            [codeExample]="nonLinearCode"
          >
      ...
      <div api class="space-y-6">
        ...
        <div role="tablist" class="tabs tabs-box tabs-boxed">
          ...
          <button role="tab" class="tab" [class.tab-active]="apiTab() === 'step-builder'" (click)="apiTab.set('step-builder')">
            Step Builder
          </button>
            ...
            <div class="card card-border card-bordered bg-base-100">
              <div class="card-body gap-3">
                ...
                <p class="text-sm text-base-content/70">
                  The <code>step.create()</code> and <code>step.review()</code> helpers create step definitions for use inside
                  <code>createForm({{ '{' }} steps: [...] {{ '}' }})</code>. Each step groups related fields together and supports an
          ...
          <div class="space-y-6">
            <div class="card card-border card-bordered bg-base-100">
              <div class="card-body gap-3">
                ...
                <p class="text-sm text-base-content/70">
                  Emitted by the <code>(stepChange)</code> output whenever the active step changes. Contains both the previous and current
                  step indices.
                </p>
            ...
            <div class="card card-border card-bordered bg-base-100">
              <div class="card-body gap-3">
                ...
                <p class="text-sm text-base-content/70">
                  Describes a single step in the wizard. Created internally by the <code>step.create()</code> and
                  <code>step.review()</code> builder functions and consumed by the stepper component.
                </p>
  ...
  stepperMethodDocs: ApiDocEntry[] = [
    {
      ...
      type: 'void',
      description:
        'Navigate to a specific step by its zero-based index. In linear mode, only allows navigation to completed steps or the next incomplete step.',
    },
    {
      name: 'onNext()',
      type: 'void',
      description: 'Navigate to the next step. When validateStepOnNext is enabled, validates the current step before allowing navigation.',
    },
    {
      name: 'onPrevious()',
      type: 'void',
      description: 'Navigate to the previous step. Always allowed regardless of linear mode or validation settings.',
    },
    {
      name: 'onComplete()',
      type: 'void',
      description: 'Mark the stepper as complete and trigger the form onSubmit callback with all collected data from every step.',
    },
    {
      name: 'isStepCompleted(index)',
      type: 'boolean',
      description: 'Returns whether the step at the given index has been completed (all its fields pass validation).',
    },
    {
      name: 'isStepActive(index)',
      type: 'boolean',
      description: 'Returns whether the step at the given index is currently visible and active.',
    },
    {
      name: 'canNavigateToStep(index)',
      type: 'boolean',
      description:
        'Returns whether navigation to the given step index is permitted based on the current linear mode and step completion state.',
    },
  ];

  stepBuilderDocs: ApiDocEntry[] = [
    {
      name: 'step.create(key, label, fields, opts?)',
      type: 'StepDefinition',
      description:
        'Create a standard wizard step with a unique key, display label, array of field definitions, and optional configuration. Fields are validated as a group when the user navigates forward.',
    },
    {
      name: 'step.review(key, label)',
      type: 'StepDefinition',
      description:
        'Create a review step that auto-generates a summary of all previously entered data. Typically placed as the last step so users can verify their inputs before submitting.',
    },
  ];
  ...
  stepperConfigDocs: ApiDocEntry[] = [
    {
      ...
      default: 'true',
      description:
        'Enforce sequential step completion. When true, users must complete each step before advancing. When false, users can jump freely between steps.',
    },
    {
      name: 'validateStepOnNext',
      type: 'boolean',
      default: 'true',
      description:
        'Run field validation on the current step when the user clicks Next. Invalid fields are highlighted and navigation is blocked until errors are resolved.',
    },
    {
      name: 'showStepSummary',
      type: 'boolean',
      default: 'true',
      description:
        'Display a summary of completed step values in the review step. Each step renders its label and the values the user entered.',
    },
  ];
  ...
  stepOptionsDocs: ApiDocEntry[] = [
    {
      ...
      default: '-',
      description:
        'Optional description text displayed below the step label in the stepper indicator. Provides context about what the step covers.',
    },
    {
      name: 'optional',
      type: 'boolean',
      default: 'false',
      description:
        'Mark the step as optional. Optional steps can be skipped without completing their fields, and the stepper indicator shows an "Optional" badge.',
    },
  ];
