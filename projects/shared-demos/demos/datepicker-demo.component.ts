    ...
    >
        ...
        @if (activeTab() === 'basic') {
          <div class="space-y-6">
            <app-doc-section title="Basic Datepicker" description="Simple single date selection" [codeExample]="basicCode">
              <div class="max-w-sm">
                <hk-datepicker [formControl]="basicControl" placeholder="Select a date" [showClearButton]="true" [showTodayButton]="true" />
              </div>
            ...
            <app-doc-section title="Date Range Picker" description="Select a start and end date" [codeExample]="rangeCode">
              <div class="max-w-md">
                <hk-datepicker [formControl]="rangeControl" [range]="true" placeholder="Select date range" [showClearButton]="true" />
              </div>
              <div class="mt-4 text-sm">
        ...
        @if (activeTab() === 'datetime') {
          <div class="space-y-6">
            <app-doc-section
              ...
              description="Select both date and time in a single dropdown"
              [codeExample]="datetimeCode"
            >
              <div class="max-w-sm">
                <hk-datepicker
          ...
          <div class="space-y-6">
            <app-doc-section title="Min/Max Constraints" description="Restrict selectable date range">
              ...
              <div class="mt-4 text-sm space-y-1">
                <div>
                  Min Date: <code class="bg-base-200 px-2 py-1 rounded">{{ minDate | date: 'mediumDate' }}</code>
                </div>
                <div>
                  Max Date: <code class="bg-base-200 px-2 py-1 rounded">{{ maxDate | date: 'mediumDate' }}</code>
                </div>
              </div>
            </app-doc-section>
            ...
            <app-doc-section title="Disabled Days of Week" description="Weekends disabled">
              <div class="max-w-sm">
                <hk-datepicker [formControl]="weekdaysControl" placeholder="Select a weekday" [disabledDaysOfWeek]="[0, 6]" />
              </div>
        ...
        @if (activeTab() === 'advanced') {
          <div class="space-y-6">
            <app-doc-section title="Custom Formatting" description="Custom date display format" [codeExample]="customFormatCode">
              <div class="max-w-sm">
                <hk-datepicker [formControl]="customFormatControl" placeholder="Select a date" [customDateFormatter]="customFormatter" />
              </div>
              <div class="mt-4 text-sm text-base-content/60">Format: "Day, Month Date, Year"</div>
            </app-doc-section>
      ...
      <div api>
        ...
        <div role="tablist" class="tabs tabs-box tabs-boxed">
          ...
          <button role="tab" class="tab" [class.tab-active]="apiTab() === 'time-mode'" (click)="apiTab.set('time-mode')">Time Mode</button>
            ...
            <div class="card card-border card-bordered bg-base-100">
              <div class="card-body gap-3">
                ...
                <p class="text-sm text-base-content/70">
                  The datepicker implements <code>ControlValueAccessor</code> and <code>Validator</code>, so it works seamlessly with
                  Angular reactive forms. The value format depends on the mode:
                </p>
                <ul class="list-disc list-inside text-sm text-base-content/70 space-y-1 ml-2">
                  <li>
                    <strong>Single date mode</strong> (default): Value is a <code>Date</code> object, or <code>null</code> when cleared.
                  </li>
                  <li>
                    <strong>Range mode</strong> (<code>[range]="true"</code>): Value is
                    <code>{{ '{' }} start: Date; end: Date {{ '}' }}</code
                    >, or <code>null</code> when cleared. The start date is always the earlier date regardless of selection order.
                  </li>
                  <li>
                    <strong>Date + time mode</strong> (<code>[showTime]="true"</code>): Value is a <code>Date</code> with hours and minutes
                    set according to the time panel selection. Seconds and milliseconds are zeroed out.
                  </li>
                </ul>
          ...
          <div class="space-y-6">
            ...
            <div class="card card-border card-bordered bg-base-100">
              <div class="card-body gap-3">
                ...
                <p class="text-sm text-base-content/70">
                  When <code>[showTime]="true"</code> is set, a time selection panel appears alongside the calendar. The user first picks a
                  date, then adjusts the hour and minute. The final value is a single <code>Date</code> object with both the date and time
                  components set.
                </p>
            ...
            <div class="card card-border card-bordered bg-base-100">
              <div class="card-body gap-3">
                ...
                <ul class="list-disc list-inside text-sm text-base-content/70 space-y-2 ml-2">
                  <li>
                    <strong>No auto-close</strong>: When <code>showTime</code> is enabled, the picker does not auto-close after date
                    selection regardless of the <code>closeOnSelect</code> setting. This ensures the user has time to adjust the hour and
                    minute before the dropdown closes.
                  </li>
                  <li>
                    <strong>Default minuteStep</strong>: The default <code>minuteStep</code> is <code>1</code>, meaning every minute is
                    selectable. For appointment-style UIs, consider setting it to <code>5</code>, <code>10</code>, <code>15</code>, or
                    <code>30</code> to reduce scrolling in the minute list.
                  </li>
                  <li>
                    <strong>Date + time combination</strong>: The component internally maintains separate signals for the selected date,
                    hour, minute, and AM/PM period. When the form value is read, these are combined into a single <code>Date</code> via
                    <code>setHours(hour, minute, 0, 0)</code>. Seconds and milliseconds are always zero.
                  </li>
                  <li>
                    <strong>12-hour vs 24-hour</strong>: By default, the time panel shows a 12-hour clock with an AM/PM toggle. Setting
                    <code>[use24Hour]="true"</code> switches to a 24-hour display (0-23) and hides the period toggle.
                  </li>
                  <li>
                    <strong>writeValue support</strong>: When a <code>Date</code> with time is written to the control (e.g., via
                    <code>patchValue</code>), the component extracts the hours and minutes from the provided date and updates the time panel
                    accordingly.
                  </li>
                </ul>
              </div>
  ...
  methodDocs: ApiDocEntry[] = [
    {
      ...
      type: 'void',
      description:
        "Toggle the calendar dropdown open or closed. Does nothing if the datepicker is disabled. When opening, resets the view to the day grid and navigates to the currently selected date's month.",
    },
    {
      name: 'openPicker()',
      type: 'void',
      description:
        'Open the calendar dropdown programmatically. Does nothing if already open or if the datepicker is disabled. Emits pickerOpened and attaches document listeners for click-outside and Escape key handling.',
    },
    {
      name: 'closePicker()',
      type: 'void',
      description:
        'Close the calendar dropdown programmatically. Marks the control as touched (which may trigger validation display) and emits pickerClosed. Removes document listeners.',
    },
    {
      name: 'selectToday()',
      type: 'void',
      description:
        "Select today's date programmatically. Does nothing if today is disabled (outside min/max range, on a disabled day of week, or in the disabledDates array). Delegates to selectDate internally.",
    },
    {
      name: 'selectDate(date)',
      type: 'void',
      description:
        'Select a specific date programmatically. In single mode, sets the date and optionally closes the picker. In range mode, starts or completes the range depending on the current state. Does nothing if the date is disabled or the component is disabled.',
    },
    {
      name: 'clearSelection()',
      type: 'void',
      description:
        'Clear the current selection, resetting the value to null. Also resets time selection (hour to 12, minute to 0, period to AM) when showTime is enabled. Marks the control as touched.',
    },
    {
      name: 'setView(mode)',
      type: 'void',
      description:
        "Change the calendar view mode. Accepts 'days', 'months', or 'years'. Emits the viewChanged output. Use this to programmatically switch between the day grid, month picker, and year picker.",
    },
    {
      name: 'goToDayView()',
      type: 'void',
      description: "Switch the calendar to the day grid view. Convenience shortcut for setView('days').",
    },
    {
      name: 'goToMonthView()',
      type: 'void',
      description: "Switch the calendar to the month picker view. Convenience shortcut for setView('months').",
    },
    {
      name: 'goToYearView()',
      type: 'void',
      description: "Switch the calendar to the year picker view. Convenience shortcut for setView('years').",
    },
    {
      name: 'navigateMonth(direction)',
      type: 'void',
      description:
        "Navigate the calendar forward or backward by one month. Accepts 'prev' or 'next'. Updates the calendar grid to show the new month's days.",
    },
    {
      name: 'navigateYears(direction)',
      type: 'void',
      description:
        "Navigate the year picker view forward or backward by one batch (24 years). Accepts 'prev' or 'next'. Only affects the year picker view.",
    },
    {
      name: 'selectYear(year)',
      type: 'void',
      description:
        'Select a year in the year picker view. Sets the calendar to that year while keeping the current month, then automatically switches to the month picker view so the user can refine their selection.',
    },
    {
      name: 'selectMonth(monthIndex)',
      type: 'void',
      description:
        'Select a month in the month picker view. Accepts a zero-based month index (0 = January, 11 = December). Sets the calendar to that month and automatically switches to the day grid view.',
    },
    {
      name: 'markAsTouched()',
      type: 'void',
      description:
        'Manually mark the control as touched. Triggers the onTouched callback for reactive form integration and may cause validation errors to display. Called automatically when the picker closes.',
    },
    {
      name: 'isDateDisabled(date)',
      type: 'boolean',
      description:
        'Check whether a specific date is disabled. Returns true if the date falls outside the min/max range, matches a disabled day of week, or is in the disabledDates array. Useful for custom UI logic.',
    },
  ];
  ...
  dateConstraintDocs: ApiDocEntry[] = [
    {
      ...
      default: 'undefined',
      description:
        'Minimum selectable date. All dates before this boundary are visually grayed out and cannot be clicked. When used with reactive forms, selecting a date before minDate produces a validation error with the key "min" containing { actual, min }.',
    },
    {
      name: 'maxDate',
      type: 'Date | undefined',
      default: 'undefined',
      description:
        'Maximum selectable date. All dates after this boundary are visually grayed out and cannot be clicked. When used with reactive forms, selecting a date after maxDate produces a validation error with the key "max" containing { actual, max }.',
    },
    {
      name: 'disabledDates',
      type: 'Date[]',
      default: '[]',
      description:
        'Array of specific dates to disable. These dates appear grayed out and cannot be selected, even if they fall within the min/max range. Date comparison is day-level only, so the time portion of each Date object is ignored.',
    },
    {
      name: 'disabledDaysOfWeek',
      type: 'number[]',
      default: '[]',
      description:
        'Array of day-of-week indices to disable across all months. 0 = Sunday, 1 = Monday, ..., 6 = Saturday. For example, [0, 6] disables all Saturdays and Sundays. These days are grayed out and non-interactive in the calendar grid.',
    },
  ];
  ...
  calendarConfigDocs: ApiDocEntry[] = [
    {
      ...
      default: 'false',
      description:
        'Show ISO week numbers in an additional column on the left side of the calendar grid. The week number is calculated using the ISO 8601 standard, where week 1 is the week containing the first Thursday of the year.',
    },
    {
      name: 'firstDayOfWeek',
      type: 'number',
      default: '0',
      description:
        'First day of the week displayed in the calendar header and grid. 0 = Sunday (US convention), 1 = Monday (ISO/European convention), etc. Affects both the weekday column headers and how day cells are laid out in each row.',
    },
  ];
  ...
  behaviorConfigDocs: ApiDocEntry[] = [
    {
      ...
      default: 'true',
      description:
        'Automatically close the picker dropdown after a date is selected. In range mode, the picker closes after both the start and end dates are chosen. This setting is ignored when showTime is true, because the picker must stay open for the user to adjust the time.',
    },
    {
      name: 'showClearButton',
      type: 'boolean',
      default: 'true',
      description:
        'Show a clear (X) icon button inside the input field. Clicking it resets the selection to null and marks the control as touched. The button only renders when there is a current selection.',
    },
    {
      name: 'showTodayButton',
      type: 'boolean',
      default: 'false',
      description:
        'Show a "Today" shortcut button in the calendar footer. Clicking it selects today\'s date and navigates the calendar to the current month. The button is automatically disabled if today is outside the min/max range or falls on a disabled day of week.',
    },
  ];
  ...
  displayConfigDocs: ApiDocEntry[] = [
    {
      ...
      default: "'Select Date'",
      description:
        'Placeholder text shown in the input field when no date is selected. Displayed as muted text to prompt the user to open the picker.',
    },
    {
      name: 'locale',
      type: 'string',
      default: "'en-US'",
      description:
        'BCP 47 locale tag that controls how dates, month names, and weekday abbreviations are formatted. Passed to Intl.DateTimeFormat internally. Changing this affects all text rendering in the calendar header, grid, and input display.',
    },
    {
      name: 'dropdownPosition',
      type: 'DatepickerPosition',
      default: "'bottom-left'",
      description:
        "Controls where the calendar dropdown appears relative to the input trigger. Options are 'bottom-left', 'bottom-right', 'top-left', and 'top-right'. Choose 'top-*' when the input is near the bottom of the viewport to prevent the calendar from being clipped.",
    },
    {
      name: 'minWidth',
      type: 'string',
      default: "'20rem'",
      description:
        'Minimum CSS width for the calendar dropdown container. Accepts any valid CSS width value (e.g., "20rem", "320px"). Increase this when using showWeekNumbers or showTime, which add extra columns to the calendar.',
    },
  ];
  ...
  customFormatterDocs: ApiDocEntry[] = [
    {
      ...
      default: 'undefined',
      description:
        'Custom function to format the selected date for display in the input field. Receives the selected Date and should return a formatted string. When provided, this completely overrides the default locale-based Intl.DateTimeFormat rendering.',
    },
    {
      name: 'customRangeFormatter',
      type: '(start: Date, end: Date) => string',
      default: 'undefined',
      description:
        'Custom function to format the selected date range for display in the input field. Receives two Date objects (start and end) and should return a formatted string. Only used when range mode is enabled. Falls back to the default "start - end" format if not provided.',
    },
  ];
  ...
  formIntegrationDocs: ApiDocEntry[] = [
    {
      ...
      default: 'false',
      description:
        'Mark the datepicker as required for form validation. When true and no date is selected, the Validator interface produces a "required" validation error. Works with both reactive forms (FormControl/FormGroup) and standalone usage.',
    },
    {
      name: 'name',
      type: 'string',
      default: "''",
      description:
        'Name attribute applied to the datepicker input element. Used for native form submission and as part of the generated unique input ID (datepicker-{name}-{instanceId}). Also contributes to ARIA labeling.',
    },
    {
      name: 'formControlName',
      type: 'string',
      default: "''",
      description:
        'Alternative to [formControl] binding. Links the datepicker to a control within a parent FormGroup by name. Used to generate the input ID when no name input is provided. Cannot be used simultaneously with [formControl].',
    },
  ];
  ...
  timeInputDocs: ApiDocEntry[] = [
    {
      ...
      default: 'false',
      description:
        'Enable time selection alongside the calendar. When true, the calendar dropdown includes a time panel with hour and minute selectors. The form value becomes a Date with both date and time components. The picker will not auto-close after date selection, giving the user time to set the hours and minutes.',
    },
    {
      name: 'use24Hour',
      type: 'boolean',
      default: 'false',
      description:
        'Switch the time panel to 24-hour format. When false (default), hours display as 1-12 with an AM/PM toggle button. When true, hours display as 0-23 and the AM/PM toggle is hidden. Only takes effect when showTime is true.',
    },
    {
      name: 'minuteStep',
      type: 'number',
      default: '1',
      description:
        'Controls the granularity of the minute selector. A step of 1 shows all 60 minutes, a step of 5 shows :00, :05, :10, ..., :55, and a step of 15 shows :00, :15, :30, :45. The computed minute options are generated as Array.from({ length: ceil(60/step) }). Only takes effect when showTime is true.',
    },
  ];
  ...
  timeMethodDocs: ApiDocEntry[] = [
    {
      ...
      type: 'void',
      description:
        'Set the selected hour programmatically. In 12-hour mode, pass the display hour (1-12) and the component converts it to 24-hour internal representation using the current AM/PM period. In 24-hour mode, pass the hour directly (0-23). Emits a value change if a date is already selected.',
    },
    {
      name: 'selectMinute(minute)',
      type: 'void',
      description:
        'Set the selected minute programmatically. Pass a minute value (typically a multiple of minuteStep, but any 0-59 value is accepted). Emits a value change if a date is already selected.',
    },
    {
      name: 'togglePeriod()',
      type: 'void',
      description:
        'Toggle between AM and PM in 12-hour mode. Adjusts the internal 24-hour value accordingly (adds or subtracts 12). Has no effect in 24-hour mode. Emits a value change if a date is already selected.',
    },
  ];
