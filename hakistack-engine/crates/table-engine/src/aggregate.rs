...
use engine_core::{FxHashSet, Idx, bitset::Bitset};
...
fn avg_number(c: &NumberColumn, rows: RowSet<'_>, n: u32) -> AggResult {
    ...
    if count > 0 {
        AggResult::Number(total / count as f64)
    } else {
        AggResult::None
    }
}
...
fn true_count(c: &BoolColumn, rows: RowSet<'_>, n: u32) -> u32 {
    let mut count = 0u32;
    for_each_index(rows, n, |i| {
        if c.validity.get(i) && c.values[i as usize] {
            count += 1;
        }
    });
    count
...
mod tests {
    ...
    fn ds() -> Dataset {
        Dataset::builder(6)
            ...
            .add_date(
                3,
                vec![Some(100), Some(200), Some(300), None, Some(200), Some(400)],
            )
            .build()
    }
    ...
    fn sum_skips_nulls() {
        // 10 + 20 + 30 + 40 + 20 = 120
        assert_eq!(
            compute(&ds(), 1, RowSet::All, AggFn::Sum),
            AggResult::Number(120.0)
        );
    }
    ...
    fn avg_uses_count_of_valid() {
        // 120 / 5 = 24
        assert_eq!(
            compute(&ds(), 1, RowSet::All, AggFn::Avg),
            AggResult::Number(24.0)
        );
    }
    ...
    fn sum_empty_input_is_none() {
        ...
        assert_eq!(
            compute(&ds(), 1, RowSet::Indices(&empty), AggFn::Sum),
            AggResult::None
        );
    }
    ...
    fn sum_all_nulls_is_none() {
        ...
        assert_eq!(
            compute(&null_only, 0, RowSet::All, AggFn::Sum),
            AggResult::None
        );
    }
    ...
    fn min_max_on_numbers() {
        assert_eq!(
            compute(&ds(), 1, RowSet::All, AggFn::Min),
            AggResult::Number(10.0)
        );
        assert_eq!(
            compute(&ds(), 1, RowSet::All, AggFn::Max),
            AggResult::Number(40.0)
        );
    }
    ...
    fn count_excludes_nulls() {
        ...
        assert_eq!(
            compute(&ds(), 1, RowSet::All, AggFn::Count),
            AggResult::Count(5)
        );
    }
    ...
    fn median_odd_length() {
        let ds = Dataset::builder(5)
            .add_number(
                0,
                vec![Some(1.0), Some(3.0), Some(5.0), Some(2.0), Some(4.0)],
            )
            .build();
        assert_eq!(
            compute(&ds, 0, RowSet::All, AggFn::Median),
            AggResult::Number(3.0)
        );
    }
    ...
    fn distinct_count_numbers() {
        ...
        assert_eq!(
            compute(&ds(), 1, RowSet::All, AggFn::DistinctCount),
            AggResult::Count(4)
        );
    }
    ...
    fn min_max_on_dates() {
        assert_eq!(
            compute(&ds(), 3, RowSet::All, AggFn::Min),
            AggResult::Date(100)
        );
        assert_eq!(
            compute(&ds(), 3, RowSet::All, AggFn::Max),
            AggResult::Date(400)
        );
    }
    ...
    fn distinct_count_dates() {
        ...
        assert_eq!(
            compute(&ds(), 3, RowSet::All, AggFn::DistinctCount),
            AggResult::Count(4)
        );
    }
    ...
    fn true_false_counts() {
        ...
        assert_eq!(
            compute(&ds(), 2, RowSet::All, AggFn::TrueCount),
            AggResult::Count(3)
        );
        assert_eq!(
            compute(&ds(), 2, RowSet::All, AggFn::FalseCount),
            AggResult::Count(2)
        );
    }
    ...
    fn distinct_count_bool_caps_at_two() {
        assert_eq!(
            compute(&ds(), 2, RowSet::All, AggFn::DistinctCount),
            AggResult::Count(2)
        );
        let only_true = Dataset::builder(3)
            ...
            .build();
        assert_eq!(
            compute(&only_true, 0, RowSet::All, AggFn::DistinctCount),
            AggResult::Count(1)
        );
    }
    ...
    fn distinct_count_text_is_case_sensitive() {
        // "a", "b", "a", "c", "b", "a"  →  {"a", "b", "c"} = 3
        assert_eq!(
            compute(&ds(), 0, RowSet::All, AggFn::DistinctCount),
            AggResult::Count(3)
        );
    }
    ...
    fn count_text_excludes_nulls() {
        let with_nulls = Dataset::builder(4)
            ...
            .build();
        assert_eq!(
            compute(&with_nulls, 0, RowSet::All, AggFn::Count),
            AggResult::Count(2)
        );
    }
    ...
    fn aggregate_over_filter_mask() {
        ...
        assert_eq!(
            compute(&ds, 1, RowSet::Mask(&mask), AggFn::Sum),
            AggResult::Number(110.0)
        );
    }
    ...
    fn aggregate_over_specific_indices() {
        ...
        assert_eq!(
            compute(&ds(), 1, RowSet::Indices(&idxs), AggFn::Sum),
            AggResult::Number(30.0)
        );
        assert_eq!(
            compute(&ds(), 1, RowSet::Indices(&idxs), AggFn::Count),
            AggResult::Count(2)
        );
    }
    ...
    fn true_count_on_number_is_none() {
        assert_eq!(
            compute(&ds(), 1, RowSet::All, AggFn::TrueCount),
            AggResult::None
        );
    }
}
