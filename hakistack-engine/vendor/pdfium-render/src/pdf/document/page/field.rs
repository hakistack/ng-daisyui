...
impl<'a> PdfFormField<'a> {
    ...
    /// Returns a mutable reference to the underlying [PdfFormComboBoxField] for this
    /// [PdfFormField], if it has a field type of [PdfFormField::ComboBox].
    /// (hakistack patch — mirrors `as_text_field_mut`.)
    #[inline]
    pub fn as_combo_box_field_mut(&mut self) -> Option<&mut PdfFormComboBoxField<'a>> {
        match self {
            PdfFormField::ComboBox(field) => Some(field),
            _ => None,
        }
    ...
    /// Returns a mutable reference to the underlying [PdfFormListBoxField] for this
    /// [PdfFormField], if it has a field type of [PdfFormField::ListBox].
    /// (hakistack patch — mirrors `as_text_field_mut`.)
    #[inline]
    pub fn as_list_box_field_mut(&mut self) -> Option<&mut PdfFormListBoxField<'a>> {
        match self {
            PdfFormField::ListBox(field) => Some(field),
            _ => None,
        }
