...
impl<'a> PdfFormComboBoxField<'a> {
    ...
    /// Sets the value of this combo box field. For a non-editable combo box,
    /// `value` should match one of the option labels; for an editable combo box
    /// it is the typed text. The change is reflected in `value()` and persisted
    /// by [crate::pdf::document::PdfDocument::save_to_bytes()].
    ///
    /// (hakistack patch — pdfium-render exposes this for text fields but not
    /// choice fields; both share the same internal implementation.)
    #[inline]
    pub fn set_value(&mut self, value: &str) -> Result<(), crate::error::PdfiumError> {
