...
impl<'a> PdfFormListBoxField<'a> {
    ...
    /// Sets the value of this list box field by selecting the option whose label
    /// matches `value`. Reflected in `value()` and persisted by
    /// [crate::pdf::document::PdfDocument::save_to_bytes()].
    ///
    /// (hakistack patch — see [PdfFormComboBoxField::set_value].)
    #[inline]
